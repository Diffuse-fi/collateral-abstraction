...
contract DataFeedFeeder {
    ...
    function set_zk(
        ...
        my_function(output, pair_names, prices, timestamps);
    }
    ...
    function my_function(
        ...
        bytes32 hashedChainId = keccak256(abi.encodePacked(chainId));
        bytes32 hashedStableAmount = keccak256(abi.encodePacked(stablecoinsAmount));

        bytes memory concatenated;
        concatenated = abi.encodePacked(concatenated, hashedDepositor);
        concatenated = abi.encodePacked(concatenated, hashedChainId);
        concatenated = abi.encodePacked(concatenated, hashedStableAmount);
        ...
        mintSyntheticStable(depositor, StableAmount); // need to figure out ERC20 here
        // emit event to confirm success on the source chain
        // need to somehow track source shain using chain-id to call slashing on the right chain for example
